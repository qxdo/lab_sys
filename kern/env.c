...
//map_segment 函数在该页表中将内核数组 pages 和 envs 映射到了用户空间的 UPAGES 和 UENVS 处。
	assert(pa % BY2PG == 0);
	assert(va % BY2PG == 0);
	assert(size % BY2PG == 0);
...
int envid2env(u_int envid, struct Env **penv, int checkperm) {
	 ...
	 *   You may want to use 'ENVX'.
	 #define ENVX(envid) ((envid) & (NENV - 1))这就是envid的索引值
	 */
	 ...
	 */
	/* 检查 'checkperm' 何时非零,检查调用环境是否有足够的权限来操作指定的环境，
	即 'e' 是 'curenv' 或其直接子代。如果违反，返回'-E_BAD_ENV'。
...
void env_init(void) {
	...
	base_pgdir = (Pde *)page2kva(p);//数为模板页表 base_pgdir 分配了一页物理内存
...
int env_alloc(struct Env **new, u_int parent_id) {
	...
	LIST_REMOVE(e,env_link);//"LIST_REMOVE" requires 2 arguments, but only 1 given
	*new = e;//printk("270   \n");
	return 0;
}
...
static void load_icode(struct Env *e, const void *binary, size_t size) {
	 ...
	 */
	size_t ph_off;//printk("322   \n");
	ELF_FOREACH_PHDR_OFF (ph_off, ehdr) {//printk("323   \n");
		Elf32_Phdr *ph = (Elf32_Phdr *)(binary + ph_off);
		if (ph->p_type == PT_LOAD) {//printk("325   \n");
...
struct Env *env_create(const void *binary, size_t size, int priority) {
	...
	e->env_pri=priority;//printk("354   \n");
	e->env_status=ENV_RUNNABLE;//printk("355   \n");
	...
	load_icode(e,binary,size);//printk("359   \n");//(struct Env *e, const void *binary, size_t size)
	TAILQ_INSERT_HEAD(&env_sched_list,e,env_sched_link);//printk("360   \n");
	return e;
}
...
void env_run(struct Env *e) {
	...
	env_pop_tf(&(curenv->env_tf),curenv->env_asid);//struct Trapframe *tf, u_int asid
}

void env_check() {
	struct Env *pe, *pe0, *pe1, *pe2;
	struct Env_list fl;
	u_long page_addr;//printk("493 j\n");
	/* should be able to allocate three envs */
	pe0 = 0;
	pe1 = 0;
	pe2 = 0;
	assert(env_alloc(&pe0, 0) == 0);printk("501 000\n");
