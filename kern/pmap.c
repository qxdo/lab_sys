...
void page_init(void) {
	...
	int i=0;
	int unavailable_npage=PPN(PADDR(freemem));
	for(i=0;i<unavailable_npage;i++)
	{
		pages[i].pp_ref=1;
	}
	...
	//printk("113 %x\n",freemem);
// 	for(p=pages;page2pa(p) <PADDR(freemem) ;p++)
// 		p->pp_ref=1;
...
int page_alloc(struct Page **new) {
	...
	memset((int *)page2kva(pp),0,BY2PG);
	*new = pp;
	return 0;
}
...
int page_insert(Pde *pgdir, u_int asid, struct Page *pp, u_long va, u_int perm) {
	...
	if (pte && (*pte & PTE_V)) {
		//printk("224aa\n");
		if (pa2page(*pte) != pp) {//printk("225aa\n");
			page_remove(pgdir, asid, va);
		} else {//printk("227aa\n");
			tlb_invalidate(asid, va);
			*pte = page2pa(pp) | perm | PTE_V;//printk("229aa\n");
			return 0;
		}
	}
	//应该做到 ：page_insert(boot_pgdir, 0, pp1, 0x0, 0) < 0
...
void physical_memory_manage_check(void) {
	...
	*temp = 1000;
	...
	assert(page_alloc(&pp0) == 0);
	assert(pp0);
	//printk("The number in address pp0 is %d\n", (int *)page2kva(pp0));
