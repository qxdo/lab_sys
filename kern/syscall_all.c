 ...
 *现在我能理解，就是现在创建了一个子进程
 	把父进程的运行上下文赋值给子进程
	让子v0寄存器的值=0，用来区分父子进程
	状态设为not runable,因为不是立即执行，还需要做其他的？
	还有些其他属性初始化为父进程的属性
 * Hint:
 ...
 */
int sys_ipc_recv(u_int dstva) {//printk("331\n");
	...
	curenv->env_ipc_recving=1;//将自身的 env_ipc_recving 设置为 1，表明该进程准备接受发送方的消息
	/* Step 3: Set the value of 'curenv->env_ipc_dstva'. */
	/* Exercise 4.8: Your code here. (2/8) */
	curenv->env_ipc_dstva=dstva;// env_ipc_dstva 赋值，表明自己要将接受到的页面与 dstva 完成映射
	/* Step 4: Set the status of 'curenv' to 'ENV_NOT_RUNNABLE' and remove it from
	 * 'env_sched_list'. */
	/* Exercise 4.8: Your code here. (3/8) */
	curenv->env_status=ENV_NOT_RUNNABLE;
	TAILQ_REMOVE(&env_sched_list, (curenv), env_sched_link);//printk("348\n");
	/* Step 5: Give up the CPU and block until a message is received. */
	((struct Trapframe *)KSTACKTOP - 1)->regs[2] = 0;//printk("350\n");
	schedule(1);//printk("351\n");
}

/* Overview:
 *   Try to send a 'value' (together with a page if 'srcva' is not 0) to the target env 'envid'.
 *把value和srcva（不为0）对应的页面发送到e。
 * Post-Condition:
...
int sys_ipc_try_send(u_int envid, u_int value, u_int srcva, u_int perm) {
	...
	if(e->env_ipc_recving==0)//不能接收信息
		return -E_IPC_NOT_RECV;
	/* Step 4: Set the target's ipc fields. */
	e->env_ipc_value = value;
	e->env_ipc_from = curenv->env_id;
	e->env_ipc_perm = PTE_V | perm;
	e->env_ipc_recving = 0;//接收完毕，修改接收状态

	/* Step 5: Set the target's status to 'ENV_RUNNABLE' again and insert it to the tail of
	 * 'env_sched_list'. */
	/* Exercise 4.8: Your code here. (7/8) */
	e->env_status=ENV_RUNNABLE;//恢复调度队列
	TAILQ_INSERT_TAIL(&env_sched_list,(e),env_sched_link);//TAILQ_INSERT_TAIL(head, elm, field)   
...
int sys_write_dev(u_int va, u_int pa, u_int len) {
	...
	if(is_illegal_va_range(va,len)||va%4!=0||pa%4!=0)
		return -E_INVAL;
	//printk("464\n");
	//printk("pa = %x pa+len= %x \n",pa,pa+len);
	if(!((pa>=0x10000000)&&(pa+len<=0x10000020))
		&&!((pa>=0x13000000)&&(pa+len<=0x13004200))
		&&!((pa>=0x15000000)&&(pa+len<=0x15000200)))
		return -E_INVAL;
	memcpy((void *)(pa|KSEG1),(const void *)va,len);
	return 0;
}
//取地址：说明要的是va 的值
//强制类型转换：要的是把va当作地址
...
int sys_read_dev(u_int va, u_int pa, u_int len) {
	...
	if(!((pa>=0x10000000)&&(pa+len<=0x10000020))
		...
		&&!((pa>=0x15000000)&&(pa+len<=0x15000200)))
		return -E_INVAL;
	memcpy((void *)va,(const void *)(pa|KSEG1),len);
	return 0;
}

void *syscall_table[MAX_SYSNO] = {//系统调用表，对应了系统异常处理的函数
 ...
 * stack.
 *调用位于syscall_table的sysno，参数来自用户态和栈
 * Hint:
 ...
 *   Number of arguments cannot exceed 5.
 使用a0寄存器的sysno来解析syscall，参数都在a1,a2,a3，参数数量不超过5
 */
void do_syscall(struct Trapframe *tf) {//这里就是调用了系统的函数
	int (*func)(u_int, u_int, u_int, u_int, u_int);//这里声明了一个函数
...
//
	...
	func=syscall_table[sysno];//异常调用函数
